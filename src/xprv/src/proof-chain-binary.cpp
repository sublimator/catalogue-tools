...
static constexpr uint8_t TLV_ANCHOR_UNL = 0x05;  // VL blob, goes at end
...
/// Encode anchor core: ledger hash + publisher key + validations.
/// This goes at the front — the verifier can check signatures against
/// a cached UNL without needing the full VL blob.
static std::vector<uint8_t>
encode_anchor_core(AnchorData const& a)
{
    ...
    // Decomposed validations: common fields once + unique per validator
...
/// Encode anchor UNL: publisher manifest + blob signature + decomposed VL blob.
/// This goes at the end — only needed if the verifier doesn't have a cached UNL.
static std::vector<uint8_t>
encode_anchor_unl(AnchorData const& a)
{
    std::vector<uint8_t> payload;
    ...
    // Decomposed VL blob with shape byte for key ordering
    if (!a.blob.empty())
    {
        std::string blob_str(
        ...
        {
            val_perm =
                detect_key_order(validators[0].as_object(), VAL_KEYS, 2);
        }
        ...
        for (auto const& v : validators)
        {
            ...
            auto vpk = hex_decode(std::string_view(
                vobj.at("validation_public_key").as_string()));
            if (vpk.size() < 33)
                vpk.resize(33, 0);
...
{
    std::vector<uint8_t> out;
    std::vector<uint8_t> unl_tail;  // UNL TLV(s), appended at end
    ...
    for (auto const& step : chain.steps)
    {
        std::visit(
            [&](auto const& data) {
                ...
                {
                    write_tlv(out, TLV_ANCHOR, encode_anchor_core(data));
                    // UNL goes at the very end
                    auto unl = encode_anchor_unl(data);
                    if (!unl.empty())
                    {
                        write_tlv(unl_tail, TLV_ANCHOR_UNL, unl);
                    }
                }
    ...
    // Append UNL data at the very end
    out.insert(out.end(), unl_tail.begin(), unl_tail.end());
...
/// Decode anchor core: ledger hash + publisher key + validations.
static AnchorData
decode_anchor_core(std::span<const uint8_t> payload)
{
    AnchorData a;
    size_t pos = 0;
...
/// Decode anchor UNL: manifest + blob signature + decomposed VL blob.
/// Merges into an existing AnchorData (from decode_anchor_core).
static void
decode_anchor_unl(std::span<const uint8_t> payload, AnchorData& a)
{
    size_t pos = 0;
...
/// Anchor core (0x01) and anchor UNL (0x05) are separate TLV records
/// that get merged into a single AnchorData step.
static ProofChain
decode_tlv_body(std::span<const uint8_t> data)
{
    ...
    // Track the anchor index so we can merge UNL into it later
    int anchor_step_idx = -1;
    ...
    while (pos < data.size())
    {
        ...
        {
            case TLV_ANCHOR:
                anchor_step_idx = static_cast<int>(chain.steps.size());
                chain.steps.push_back(decode_anchor_core(payload));
                break;

            case TLV_ANCHOR_UNL:
                // Merge into the existing anchor step
                if (anchor_step_idx >= 0)
                {
                    auto& anchor = std::get<AnchorData>(
                        chain.steps[anchor_step_idx]);
                    decode_anchor_unl(payload, anchor);
                }
                break;
