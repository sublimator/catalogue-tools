...
cmd_prove(ProveOptions const& opts, catl::xdata::Protocol const& protocol)
{
    ...
    auto engine = xprv::ProofEngine::create(io, std::move(config));
    ...
    boost::asio::co_spawn(
        ...
        [&, engine]() -> boost::asio::awaitable<void> {
            ...
            {
                auto binary =
                    xprv::to_binary(chain, {.compress = opts.compress});
            ...
            {
                auto chain_json = xprv::to_json(chain);
                ...
                auto compressed = xprv::to_binary(chain, {.compress = true});
                bin_path = output_stem + ".bin";
