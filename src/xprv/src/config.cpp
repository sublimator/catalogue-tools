...
config_file_path()
{
    ...
    {
        auto p = fs::path(home) / ".config" / "xprv" / "config.toml";
        if (fs::exists(p))
            return p.string();
    }
...
{
    if (auto v = env_u32("XPRV_NETWORK_ID"))
        config.network_id = *v;
    if (auto v = env_str("XPRV_RPC_HOST"))
        config.rpc_host = *v;
    if (auto v = env_u16("XPRV_RPC_PORT"))
        config.rpc_port = *v;
    if (auto v = env_str("XPRV_PEER_HOST"))
        config.peer_host = *v;
    if (auto v = env_u16("XPRV_PEER_PORT"))
        config.peer_port = *v;
    if (auto v = env_str("XPRV_VL_HOST"))
        config.vl_host = *v;
    if (auto v = env_u16("XPRV_VL_PORT"))
        config.vl_port = *v;
    if (auto v = env_str("XPRV_PUBLISHER_KEY"))
        config.publisher_key = *v;
    if (auto v = env_str("XPRV_BIND"))
        config.bind_address = *v;
    if (auto v = env_u16("XPRV_PORT"))
        config.port = *v;
    if (auto v = env_u32("XPRV_THREADS"))
        config.threads = *v;
    if (auto v = env_bool("XPRV_NO_CACHE"))
        config.no_cache = *v;
...
{
    os << "# Resolved xprv configuration\n";
