...
class PermEncoding
{
    ...
    inline void
    unpack(uint16_t in, uint8_t out[4]) const
    {
        out[0] = (in & 0x000f);
        out[2] = ((in >> 4) & 0x000f);
        out[1] = ((in >> 8) & 0x000f);
...
public:
    ...
    inline void
    decode(const uint16_t codeword, uint8_t lowbits[4]) const
    {
        unpack(dec_table[codeword], lowbits);
    }

    inline uint16_t
    encode(const uint8_t lowbits[4]) const
    {
    ...
    void
    gen_tables(int base, int k, uint8_t dst[4], uint16_t& idx)
    {
