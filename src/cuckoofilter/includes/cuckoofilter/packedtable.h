...
public:
    ...
    void
    PrintBucket(const size_t i) const
    {
    ...
    void
    PrintTags(uint32_t tags[4]) const
    {
        ...
        for (size_t j = 0; j < 4; j++)
        {
            ...
            std::cout << " lowbits="
                      ...
                      << " dirbits="
                      << PrintUtil::bytes_to_hex(
                             (char*)&dirbits[j], kDirBitsPerTag / 8 + 1)
                      << std::endl;
        }
    ...
    inline void
    SortPair(uint32_t& a, uint32_t& b)
    {
        if ((a & 0x0f) > (b & 0x0f))
        {
            std::swap(a, b);
     ...
     */
    inline void
    ReadBucket(const size_t i, uint32_t tags[4]) const
    {
     ...
     */
    inline void
    WriteBucket(const size_t i, uint32_t tags[4], bool sort = true)
    {
        ...
        {
            // 1 dirbits per tag
            *((uint16_t*)p) = codeword | (highbits[0] << 8) |
                (highbits[1] << 9) | (highbits[2] << 10) | (highbits[3] << 11);
        }
        ...
        else if (kBitsPerBucket == 24)
        {
            ...
            *((uint32_t*)p) |= codeword | (highbits[0] << 8) |
                (highbits[1] << 11) | (highbits[2] << 14) | (highbits[3] << 17);
        }
        ...
        else if (kBitsPerBucket == 32)
        {
            // 5 dirbits per tag
            *((uint32_t*)p) = codeword | (highbits[0] << 8) |
                (highbits[1] << 13) | (highbits[2] << 18) | (highbits[3] << 23);
            DPRINTF(
                DEBUG_TABLE,
                " new bucketbits=%s\n",
    ...
    bool
    FindTagInBuckets(const size_t i1, const size_t i2, const uint32_t tag) const
    {
        ...
        uint16_t v;
        uint64_t bucketbits1 =
            *((uint64_t*)(buckets_ + kBitsPerBucket * i1 / 8));
        uint64_t bucketbits2 =
            *((uint64_t*)(buckets_ + kBitsPerBucket * i2 / 8));
    ...
    bool
    FindTagInBucket(const size_t i, const uint32_t tag) const
    {
        DPRINTF(DEBUG_TABLE, "PackedTable::FindTagInBucket %zu\n", i);
        uint32_t tags[4];
        ReadBucket(i, tags);
    ...
    bool
    DeleteTagFromBucket(const size_t i, const uint32_t tag)
    {
        uint32_t tags[4];
        ReadBucket(i, tags);
        if (debug_level & DEBUG_TABLE)
        ...
        uint32_t& oldtag)
    {
        ...
        if (kickout)
        {
            ...
            DPRINTF(
                DEBUG_TABLE,
                "PackedTable::InsertTagToBucket, let's kick out a random slot "
                "%zu \n",
                r);
            // PrintBucket(i);
        ...
        DPRINTF(
            DEBUG_TABLE, "PackedTable::InsertTagToBucket, insert failed \n");
        return false;
    }
