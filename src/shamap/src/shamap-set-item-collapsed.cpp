...
void
SHAMap::configure_cow_collapsed(PathFinder pathFinder)
{
...
SetResult
SHAMap::handle_collision_collapsed(
    ...
    PathFinder pathFinder) const
{
    ...
    uint8_t currentDepth = parent->get_depth() + 1;  // Start depth below parent

    // Create first new inner node to replace the leaf
    auto newInner = boost::intrusive_ptr(new SHAMapInnerNode(currentDepth));
    if (cow_enabled_)
    {
    ...
    {
        // Max depth check
        int existingBranch = select_branch(existingItem->key(), currentDepth);
        ...
        else
        {
            ...
            OLOGD(
                ...
                ". Descending further.");
            auto nextInner =
                boost::intrusive_ptr(new SHAMapInnerNode(currentDepth + 1));
            if (cow_enabled_)
            {
...
SHAMap::set_item_collapsed(boost::intrusive_ptr<MmapItem>& item, SetMode mode)
{
    ...
    try
    {
        ...
        configure_cow_collapsed(pathFinder);
        ...
        {
            return handle_collision_collapsed(item, pathFinder);
        }
    ...
    {
        OLOGD("Error adding item with key ", item->key().hex(), ": ", e.what());
        return SetResult::FAILED;
    }
