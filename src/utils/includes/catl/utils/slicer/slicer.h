...
public:
     ...
     */
    void
    set_item(const Key& key, const uint8_t* data, uint32_t size);
     ...
     */
    size_t
    serialize(catl::v1::Writer& writer) const;
...
private:
     ...
     */
    std::string
    get_snapshot_filename(uint32_t ledger_seq) const;
...
protected:
    /**
     * Copy a state map delta from input to output and update state if needed
     *
     * @param ledger_seq Ledger sequence number for logging
     * @return Number of bytes copied
     */
    size_t
    copy_state_map_delta(uint32_t ledger_seq);

    /**
     * Copy a transaction map from input to output
     *
     * @return Number of bytes copied
     */
    size_t
    copy_transaction_map();
};
