...
void InMemoryStateMap::set_item(const Key& key, const uint8_t* data, uint32_t size) {
    std::vector<uint8_t> value_data(data, data + size);
    items_[key] = std::move(value_data);
}

bool InMemoryStateMap::remove_item(const Key& key) {
...
size_t InMemoryStateMap::size() const {
    return items_.size();
}

size_t InMemoryStateMap::serialize(catl::v1::Writer& writer) const {
    size_t item_count = 0;
    
    for (const auto& [key, data] : items_) {
        // Write each item as: tnACCOUNT_STATE + key + data
        uint8_t type = catl::SHAMapNodeType::tnACCOUNT_STATE;
        writer.write_raw_data(&type, sizeof(type));
        writer.write_raw_data(key.data(), Key::size());
        
        // Write data length as uint32_t
        uint32_t data_size = static_cast<uint32_t>(data.size());
        writer.write_raw_data(reinterpret_cast<const uint8_t*>(&data_size), sizeof(data_size));
    ...
    uint8_t terminal = catl::SHAMapNodeType::tnTERMINAL;
    writer.write_raw_data(&terminal, sizeof(terminal));
    
    return item_count;
}

const std::map<Key, std::vector<uint8_t>>& InMemoryStateMap::items() const {
    return items_;
}
...
Slicer::Slicer(const CommandLineOptions& options) 
    ...
    if (options_.snapshots_path) {
        ...
        fs::path output_dir = fs::path(options_.output_file.value()).parent_path();
...
bool Slicer::run() {
    ...
    try {
        ...
        if (options_.create_next_slice_state_snapshot || options_.use_start_snapshot) {
        ...
        if (options_.create_next_slice_state_snapshot) {
            if (!create_next_slice_snapshot()) {
                LOGW("Failed to create snapshot for next slice, but slice creation was successful");
            }
        }
        ...
        LOGI("  Start snapshot used:", stats_.start_snapshot_used ? "Yes" : "No");
        LOGI("  End snapshot created:", stats_.end_snapshot_created ? "Yes" : "No");
...
bool Slicer::validate_ledger_ranges() {
    ...
    if (start < header.min_ledger || end > header.max_ledger) {
        LOGE("Requested ledger range [", start, "-", end, 
    ...
    if (start > end) {
        LOGE("Start ledger (", start, ") must be less than or equal to end ledger (", end, ")");
        return false;
    }
...
bool Slicer::process_first_ledger() {
    ...
    if (!use_snapshot_success) {
        LOGI("No suitable snapshot found or snapshot usage disabled, fast-forwarding through input");
...
bool Slicer::try_use_start_snapshot() {
    uint32_t start_ledger = options_.start_ledger.value();
    ...
    try {
        ...
        while (!found_start_ledger) {
            try {
                ...
                LOGE("Failed to read ledger info while looking for start ledger:", e.what());
                return false;
            }
        }
        ...
        // Create in-memory state map if we need to save the next slice's snapshot
        if (options_.create_next_slice_state_snapshot) {
        ...
        while (decomp_stream) {
            decomp_stream.read(reinterpret_cast<char*>(buffer.data()), buffer_size);
            ...
            if (bytes_read > 0) {
                ...
                if (state_map_) {
                    // This is simplistic - in reality, we would need to keep track of partial
                    // nodes and accumulate data across buffer boundaries. This is just to
                    // illustrate the concept.
                    LOGW("Parsing snapshot stream to populate state map not fully implemented");
                    // TODO: Implement proper parsing of the snapshot data to populate state_map_
                }
            }
        }
        ...
        while (!found_terminal) {
            if (reader_->read_raw_data(&node_type, 1) != 1) {
                LOGE("Unexpected end of file while looking for terminal marker");
                return false;
            }
            
            if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
                found_terminal = true;
                ...
                if (reader_->read_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4) != 4) {
                ...
                if (reader_->read_raw_data(skip_buffer.data(), data_length) != data_length) {
        ...
        while (!found_terminal) {
            ...
            if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
            ...
            } else if (node_type == catl::SHAMapNodeType::tnTRANSACTION_NM || 
                      node_type == catl::SHAMapNodeType::tnTRANSACTION_MD) {
                // Copy key
                uint8_t key_data[32];
                if (reader_->read_raw_data(key_data, 32) != 32) {
                ...
                writer_->write_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4);
                total_bytes += 4;
                
                // Copy the data
                std::vector<uint8_t> data_buffer(data_length);
                if (reader_->read_raw_data(data_buffer.data(), data_length) != data_length) {
                ...
                LOGE("Unexpected node type in tx map:", static_cast<int>(node_type));
                return false;
            }
        }
...
bool Slicer::fast_forward_to_start() {
    ...
    try {
        ...
        for (uint32_t current_seq = input_min_ledger; current_seq < start_ledger; current_seq++) {
            ...
            if (ledger_info.sequence != current_seq) {
                LOGE("Expected ledger", current_seq, "but found", ledger_info.sequence);
                return false;
            }
            ...
            while (!state_map_done) {
                ...
                if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
                    state_map_done = true;
                } else if (node_type == catl::SHAMapNodeType::tnACCOUNT_STATE) {
                    // Read key
                    uint8_t key_data[32];
                    if (reader_->read_raw_data(key_data, 32) != 32) {
                        LOGE("Unexpected end of file while reading key");
                    ...
                    if (reader_->read_raw_data(data.data(), data_length) != data_length) {
                    ...
                    stats_.state_items_processed++;
                } else if (node_type == catl::SHAMapNodeType::tnREMOVE) {
                    ...
                    LOGE("Unexpected node type in state map:", static_cast<int>(node_type));
                    return false;
                }
            }
        ...
        catl::v1::LedgerInfo ledger_info = reader_->read_ledger_info();
        if (ledger_info.sequence != start_ledger) {
            LOGE("Expected ledger", start_ledger, "but found", ledger_info.sequence);
            return false;
        }
        ...
        LOGI("Writing full state map for ledger", start_ledger, "with", state_map_->size(), "items");
...
bool Slicer::process_subsequent_ledgers() {
    ...
    try {
        for (uint32_t current_seq = start_ledger + 1; current_seq <= end_ledger; current_seq++) {
...
size_t Slicer::copy_state_map_delta(uint32_t ledger_seq) {
    ...
    while (!done) {
        uint8_t node_type;
        if (reader_->read_raw_data(&node_type, 1) != 1) {
            throw std::runtime_error("Unexpected end of file while copying state map delta");
        }
        ...
        if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
            ...
            uint8_t key_data[32];
            if (reader_->read_raw_data(key_data, 32) != 32) {
                throw std::runtime_error("Unexpected end of file while reading state key");
            }
            writer_->write_raw_data(key_data, 32);
            bytes_copied += 32;
            ...
            uint32_t data_length;
            if (reader_->read_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4) != 4) {
                throw std::runtime_error("Unexpected end of file while reading state data length");
            }
            writer_->write_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4);
            bytes_copied += 4;
            ...
            std::vector<uint8_t> data_buffer(data_length);
            if (reader_->read_raw_data(data_buffer.data(), data_length) != data_length) {
                throw std::runtime_error("Unexpected end of file while reading state data");
            }
            writer_->write_raw_data(data_buffer.data(), data_length);
            ...
            uint8_t key_data[32];
            if (reader_->read_raw_data(key_data, 32) != 32) {
                throw std::runtime_error("Unexpected end of file while reading remove key");
            }
            writer_->write_raw_data(key_data, 32);
            bytes_copied += 32;
            ...
            throw std::runtime_error("Unexpected node type in state map: " + 
...
size_t Slicer::copy_transaction_map() {
    size_t bytes_copied = 0;
    bool done = false;
    ...
    while (!done) {
        uint8_t node_type;
        if (reader_->read_raw_data(&node_type, 1) != 1) {
            throw std::runtime_error("Unexpected end of file while copying tx map");
        }
        ...
        if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
            ...
            uint8_t key_data[32];
            if (reader_->read_raw_data(key_data, 32) != 32) {
                throw std::runtime_error("Unexpected end of file while reading tx key");
            }
            writer_->write_raw_data(key_data, 32);
            bytes_copied += 32;
            ...
            uint32_t data_length;
            if (reader_->read_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4) != 4) {
                throw std::runtime_error("Unexpected end of file while reading tx data length");
            }
            writer_->write_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4);
            bytes_copied += 4;
            ...
            std::vector<uint8_t> data_buffer(data_length);
            if (reader_->read_raw_data(data_buffer.data(), data_length) != data_length) {
                throw std::runtime_error("Unexpected end of file while reading tx data");
            }
            writer_->write_raw_data(data_buffer.data(), data_length);
            bytes_copied += data_length;
        } else {
            throw std::runtime_error("Unexpected node type in tx map: " + 
                                     std::to_string(static_cast<int>(node_type)));
        }
    }
...
bool Slicer::create_next_slice_snapshot() {
    if (!state_map_) {
    ...
    try {
        ...
        LOGI("Reading state delta for ledger", next_ledger, "to create snapshot");
        
        // Read ledger header for next ledger
        catl::v1::LedgerInfo ledger_info = reader_->read_ledger_info();
        if (ledger_info.sequence != next_ledger) {
            LOGE("Expected ledger", next_ledger, "but found", ledger_info.sequence);
            return false;
        }
        ...
        while (!done) {
            uint8_t node_type;
            if (reader_->read_raw_data(&node_type, 1) != 1) {
                LOGE("Unexpected end of file while reading next ledger state map");
                return false;
            }
            ...
            if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
                ...
                uint8_t key_data[32];
                if (reader_->read_raw_data(key_data, 32) != 32) {
                    LOGE("Unexpected end of file while reading next ledger key");
                    return false;
                }
                Key key(key_data);
                ...
                uint32_t data_length;
                if (reader_->read_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4) != 4) {
                    LOGE("Unexpected end of file while reading next ledger data length");
                    return false;
                }
                
                std::vector<uint8_t> data(data_length);
                if (reader_->read_raw_data(data.data(), data_length) != data_length) {
                    LOGE("Unexpected end of file while reading next ledger data");
                    return false;
                }
                ...
                uint8_t key_data[32];
                if (reader_->read_raw_data(key_data, 32) != 32) {
                    LOGE("Unexpected end of file while reading next ledger remove key");
                    return false;
                }
                Key key(key_data);
                ...
                LOGE("Unexpected node type in next ledger state map:", static_cast<int>(node_type));
                return false;
            }
        }
        
        LOGI("Applied", items_processed, "state items from ledger", next_ledger, "to state map");
        ...
        while (!tx_map_done) {
            uint8_t node_type;
            if (reader_->read_raw_data(&node_type, 1) != 1) {
                LOGE("Unexpected end of file while skipping next ledger tx map");
                return false;
            }
            ...
            if (node_type == catl::SHAMapNodeType::tnTERMINAL) {
                ...
                uint8_t key_data[32];
                if (reader_->read_raw_data(key_data, 32) != 32) {
                    LOGE("Unexpected end of file while skipping next ledger tx key");
                    return false;
                }
                ...
                uint32_t data_length;
                if (reader_->read_raw_data(reinterpret_cast<uint8_t*>(&data_length), 4) != 4) {
                    LOGE("Unexpected end of file while reading next ledger tx data length");
                    return false;
                }
                
                std::vector<uint8_t> skip_buffer(data_length);
                if (reader_->read_raw_data(skip_buffer.data(), data_length) != data_length) {
                    LOGE("Unexpected end of file while skipping next ledger tx data");
                    return false;
                }
            } else {
                LOGE("Unexpected node type in next ledger tx map:", static_cast<int>(node_type));
                return false;
            }
        }
        ...
        comp_stream.push(io::zlib_compressor(io::zlib::best_compression)); // Use max compression
        ...
        for (const auto& [key, data] : state_map_->items()) {
            // Write each item as: tnACCOUNT_STATE + key + data
            uint8_t type = catl::SHAMapNodeType::tnACCOUNT_STATE;
            comp_stream.write(reinterpret_cast<const char*>(&type), sizeof(type));
            
            // Write key
            comp_stream.write(reinterpret_cast<const char*>(key.data()), Key::size());
            
            // Write data length as uint32_t
            uint32_t data_size = static_cast<uint32_t>(data.size());
            comp_stream.write(reinterpret_cast<const char*>(&data_size), sizeof(data_size));
            
            // Write actual data
            comp_stream.write(reinterpret_cast<const char*>(data.data()), data.size());
        ...
        // Write terminal marker
        uint8_t terminal = catl::SHAMapNodeType::tnTERMINAL;
        comp_stream.write(reinterpret_cast<const char*>(&terminal), sizeof(terminal));
        
        comp_stream.flush();
        
        LOGI("Wrote", items_written, "items to snapshot file for ledger", next_ledger);
...
std::string Slicer::get_snapshot_filename(uint32_t ledger_seq) const {
    std::ostringstream oss;
    oss << snapshots_path_ << "/state_snapshot_for_ledger_" << ledger_seq << ".dat.zst";
    return oss.str();
}

const SliceStats& Slicer::stats() const {
    return stats_;
}
