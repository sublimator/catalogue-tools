...
parse_argv(int argc, char* argv[])
{
    ...
    help_text << "catl-slice: High-Performance CATL File Slicing Tool\n\n"
    ...
    desc.add_options()
        ...
        ("input,i", po::value<std::string>(), "Path to the source CATL file (v1 format) (required)")
        ("output,o", po::value<std::string>(), "Path where the generated CATL slice file will be saved (required)")
        ("start-ledger", po::value<uint32_t>(), "The sequence number of the first ledger to include in the slice (required)")
        ("end-ledger", po::value<uint32_t>(), "The sequence number of the last ledger to include in the slice (required)")
        ("snapshots-path", po::value<std::string>(), "Directory where state snapshots are stored and looked for")
        ("compression-level", po::value<uint8_t>()->default_value(0), "Compression level (0-9, where 0 means uncompressed)")
        ("force-overwrite", "If output files exist, overwrite them without prompting")
        ("create-next-slice-state-snapshot", po::value<bool>()->default_value(true), "Create a state snapshot for the next slice")
        ("no-create-next-slice-state-snapshot", "Disable creation of a state snapshot for the next slice")
        ("use-start-snapshot", po::value<bool>()->default_value(true), "Use a start snapshot if available")
        ("no-use-start-snapshot", "Ignore any existing start snapshots")
        ("log-level", po::value<std::string>()->default_value("info"), "Log verbosity (error, warn, info, debug)");
    ...
    try {
        ...
        if (options.start_ledger > options.end_ledger) {
            options.valid = false;
            options.error_message = "Error: start-ledger must be less than or equal to end-ledger";
            return options;
        }
        ...
        if (vm.count("no-create-next-slice-state-snapshot")) {
            ...
            options.create_next_slice_state_snapshot = vm["create-next-slice-state-snapshot"].as<bool>();
        }
        ...
        if (vm.count("log-level")) {
            options.log_level = string_to_log_level(vm["log-level"].as<std::string>());
        }

        // Set default snapshots path if not provided
        if (!options.snapshots_path) {
            fs::path output_dir = fs::path(options.output_file.value()).parent_path();
            options.snapshots_path = (output_dir / "catl_snapshots").string();
        }

    } catch (const po::error& e) {
        options.valid = false;
        options.error_message = std::string("Error parsing command line: ") + e.what();
