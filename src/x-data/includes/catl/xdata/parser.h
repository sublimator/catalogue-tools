...
// TODO: none of this shit is actually used
// Simple type aliases for common XRPL types
using AccountID = std::array<uint8_t, 20>;
using Hash128 = std::array<uint8_t, 16>;
using Hash160 = std::array<uint8_t, 20>;
using Hash192 = std::array<uint8_t, 24>;
using Amount = std::array<uint8_t, 48>;  // Max size for Amount
...
struct SliceCursor {
    ...
    size_t remaining_size() const { return data.size() - pos; }
    Slice remaining() const { return data.subslice(pos); }
    ...
    Slice read_slice(size_t n) {
        if (pos + n > data.size()) {
            throw std::runtime_error("SliceCursor: attempted to read " + 
                std::to_string(n) + " bytes, only " + 
...
struct PathElement {
    ...
    // maybe just collapse this into a single int with -1 as not an array element?
...
struct FieldSlice {
    // TODO: maybe just encode the field as a slice too? or as well as the actual field def
    const FieldDef* field;
    Slice data;
};
...
template<typename T>
concept SliceVisitor = requires(T v, const FieldPath& path, const FieldDef& f, Slice s, size_t idx) {
    { v.visit_object_start(path, f) } -> std::convertible_to<bool>;
    { v.visit_object_end(path, f) } -> std::same_as<void>;
    { v.visit_array_start(path, f) } -> std::convertible_to<bool>;
    { v.visit_array_end(path, f) } -> std::same_as<void>;
    { v.visit_array_element(path, idx) } -> std::convertible_to<bool>;
    { v.visit_field(path, f, s) } -> std::same_as<void>;
};

// Forward declarations
inline void skip_array(SliceCursor& cursor, const Protocol& protocol);

// TODO: move this to a separate file with the SliceCursor ? and other little utilslike read_vl_length
// Read field header and return field code
inline uint32_t read_field_header(SliceCursor& cursor) {
    ...
    if (type == 0) {
        ...
        if (cursor.empty()) return 0;
        type = cursor.read_u8();
        if (type == 0 || type < 16) return 0;
    } else if (field == 0) {
        // Field in next byte
        if (cursor.empty()) return 0;
        field = cursor.read_u8();
        if (field == 0 || field < 16) return 0;
    }
...
inline size_t read_vl_length(SliceCursor& cursor) {
    uint8_t byte1 = cursor.read_u8();
    ...
    // Invalid VL encoding
    throw std::runtime_error("Invalid VL encoding: first byte = " + 
        std::to_string(static_cast<int>(byte1)));
}

// Get size for Amount type by peeking at first byte
inline size_t get_amount_size(const SliceCursor& cursor) {
    if (cursor.empty()) {
        throw std::runtime_error("Cannot determine Amount size: no data available");
    }
    uint8_t first_byte = cursor.data.data()[cursor.pos];
...
inline size_t get_fixed_size(FieldType type) {
    switch (type.code) {
        // TODO: use the FieldType enum instead of the code
        case 1: return 2;   // UInt16
        case 2: return 4;   // UInt32
        case 3: return 8;   // UInt64
        case 4: return 16;  // Hash128
        case 5: return 32;  // Hash256
        case 6: return 0;   // Amount - SPECIAL CASE, not fixed!
        case 8: return 20;  // AccountID
        case 16: return 1;  // UInt8
        case 17: return 20; // Hash160
        case 18: return 0;  // PathSet - SPECIAL CASE, state machine!
        case 19: return 0;  // Vector256 - VL encoded
        case 20: return 12; // UInt96
        case 21: return 24; // Hash192
        case 22: return 48; // UInt384
        case 23: return 64; // UInt512
        case 24: return 24; // Issue (20 currency + 4 reserved)
        case 25: return 40; // XChainBridge (20 + 20 account IDs)
        case 26: return 20; // Currency
        default: return 0;  // Variable or unknown
    }
}

// Skip a PathSet (has its own termination protocol)
inline void skip_pathset(SliceCursor& cursor) {
...
// Skip an entire object (find end marker) - only used when visitor returns false
inline void skip_object(SliceCursor& cursor, const Protocol& protocol) {
    while (!cursor.empty()) {
        uint32_t field_code = read_field_header(cursor);
        if (field_code == 0) break;  // Error or end
        
        const FieldDef* field = protocol.get_field_by_code(field_code);
        if (!field) break;  // Unknown field
        ...
        if (field->meta.type == FieldTypes::STObject) {
            ...
            if (fixed_size == 0) {
                throw std::runtime_error("Unknown field type size: " + 
...
void parse_with_visitor(SliceCursor& cursor, const Protocol& protocol, Visitor&& visitor) {
    FieldPath path;
...
template<SliceVisitor Visitor>
void parse_with_visitor_impl(SliceCursor& cursor, const Protocol& protocol, 
                            Visitor&& visitor, FieldPath& path) {
        ...
        if (field->meta.type == FieldTypes::STObject) {
            ...
            if (visitor.visit_array_start(path, *field)) {
                ...
                while (!cursor.empty()) {
                    ...
                    const FieldDef* peek_field = protocol.get_field_by_code(peek_code);
                    if (peek_field && peek_field->meta.type == FieldTypes::STArray && 
...
public:
    ...
    bool visit_object_start(const FieldPath&, const FieldDef&) { 
        return true;  // Always descend
    }
    void visit_object_end(const FieldPath&, const FieldDef&) {}
    
    bool visit_array_start(const FieldPath&, const FieldDef&) { 
        return true;
    }
    void visit_array_end(const FieldPath&, const FieldDef&) {}
    
    bool visit_array_element(const FieldPath&, size_t) { return true; }
    
    void visit_field(const FieldPath&, const FieldDef& field, Slice data) {
        emit_({&field, data});
    }
};
...
public:
    ...
    bool visit_object_start(const FieldPath& path, const FieldDef&) { 
    ...
    bool visit_array_element(const FieldPath&, size_t) { return false; }
    
    void visit_field(const FieldPath& path, const FieldDef& field, Slice data) {
        if (path.size() == 1) {  // Top-level field
            process_({&field, data});
        }
    }
};
