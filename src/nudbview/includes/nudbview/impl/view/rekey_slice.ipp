    ...
    Args&&... args)
{
    ...
    static_assert(
        is_Progress<Progress>::value, "Progress requirements not met");

    using namespace detail;

    // Validate inputs
    if (end_offset_incl <= start_offset_incl)
    {
    ...
    if (start_offset_incl < dat_file_header::size)
    {
        ec = error::slice_start_before_header;
        return;
    }
    if (end_offset_incl >= dataFileSize)
    {
    ...
    // -----------------------------------------------------
    // If caller already has an index and knows the exact record count (common
    // case when using IndexBuilder), they can pass expected_record_count to
    // skip this pass. We'll validate the count during Pass 2.
    //
    ...
    // IndexBuilder. If end_offset points into a PARTIAL record (incomplete
    // write), scan_dat_records will stop early when it detects the record
    // extends beyond end_offset.
    //
    ...
    //
    // For stress testing concurrent writes, see
    // tests/nudbview/slice-stress-gtest.cpp
    ...
    if (end_offset_incl >= dat_file_size)
    {
        ec = error::slice_end_exceeds_file;
        return;
    }

    progress(0, 2 * (end_offset_incl - start_offset_incl));  // 2 passes total
    ...
    if (expected_record_count > 0)
    {
        ...
        std::uint64_t expected_entries =
            (itemCount + index_interval - 1) / index_interval;
    ...
    else
    {
        ...
        itemCount = nudbutil::scan_dat_records(
            ...
            [&](std::uint64_t record_num,
                ...
                progress(
                    offset - start_offset_incl,
                    2 * (end_offset_incl - start_offset_incl));
            },
            start_offset_incl,
            0);
    }
    ...
    kh.buckets = static_cast<std::size_t>(
        std::ceil(itemCount / (bucket_capacity(kh.block_size) * loadFactor)));
    ...
    smh.start_byte_offset_incl = start_offset_incl;
    smh.end_byte_offset_incl = end_offset_incl;
    ...
    smh.spill_section_offset =
        slice_meta_header::size + index_entries.size() * index_entry::size;
    ...
    auto const chunkSize = std::max<std::size_t>(1, bufferSize / kh.block_size);
    ...
    for (nbuck_t b0 = 0; b0 < kh.buckets; b0 += chunkSize)
    {
        ...
        // If expected_record_count was provided, we also validate the actual
        // count
        ...
        nudbutil::scan_dat_records(
            ...
            [&](std::uint64_t record_num,
                ...
                std::uint64_t size) {
                if (record_offset > end_offset_incl)
                    return;
                ...
                progress(
                    (end_offset_incl - start_offset_incl) +
                        (b0 / chunkSize) *
                            (end_offset_incl - start_offset_incl) /
                ...
                std::uint8_t const* const key =
                    dat_data + record_offset + field<uint48_t>::size;
                ...
                {
                    // Save the size of the full bucket before we clear it
                    auto const full_bucket_size = b.actual_size();
                    ...
                    write<std::uint16_t>(os, full_bucket_size);
                    b.write(os);

                    ++spill_count;

                    // Clear bucket and set spill pointer
                    // The spill starts at current offset minus what we just
                    // wrote
                    b = bucket{
                        kh.block_size,
                        buf.get() + (n - b0) * kh.block_size,
                        empty};
                    b.spill(
                        mw.offset() - full_bucket_size - field<uint48_t>::size -
        ...
        kf.write(
            (b0 + 1) * kh.block_size,
            buf.get(),
            static_cast<std::size_t>(bn * kh.block_size),
            ec);
        if (ec)
            return;
    }
