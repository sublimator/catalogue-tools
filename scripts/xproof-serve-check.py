...
# ///
"""
Run one isolated xproof-serve case with immediate, readable output.

This is intentionally not pytest. It is a debug-first runner:
- starts a fresh xproof serve process for each invocation
...
Examples:
    scripts/xproof-serve-check.py health --xproof build/src/xproof/xproof
    scripts/xproof-serve-check.py prove-json --threads 4 --tx-corpus scripts/xproof-tx-corpus.generated.json
    scripts/xproof-serve-check.py verify-json-roundtrip --tx 8AF2DE8804721B0EC9E11FD66B9EF3C30962E71DC0D707CA1F4CDE4655751420
    scripts/xproof-serve-check.py concurrent-prove --concurrency 8 --threads 4
"""
    ...
    def _wait_for_port(self) -> None:
        ...
        while time.monotonic() < deadline:
            if self.proc is not None and self.proc.poll() is not None:
                raise CaseFailure(
                    "xproof serve exited before opening the port\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--xproof", default="build/src/xproof/xproof", help="Path to xproof binary")
...
def main(argv: list[str] | None = None) -> int:
    ...
    xproof = Path(args.xproof)
    if not xproof.exists():
        raise SystemExit(f"xproof binary not found: {xproof}")

    artifact_dir = (
        Path(args.artifact_dir)
        if args.artifact_dir
        else Path(tempfile.mkdtemp(prefix="xproof-serve-check-"))
    )
