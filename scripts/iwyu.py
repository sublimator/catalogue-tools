...
class IWYUScanner:
    ...
    def run_iwyu(self, file_path: Path, includes: Optional[List[str]] = None) -> IWYUResult:
    ...
    def analyze_directory(self, src_dir: Path, includes: Optional[List[str]] = None) -> List[IWYUResult]:
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Run include-what-you-use on C++ header files')
    parser.add_argument('--src-dir', type=str, default='src',
                        help='Source directory to scan (default: src)')
    parser.add_argument('--include-dirs', type=str, nargs='*',
