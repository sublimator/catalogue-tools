...
# ///
"""
Pytest integration suite for `xproof serve`.
...
Run directly:
    scripts/xproof-serve-smoke.py -q
    scripts/xproof-serve-smoke.py --xproof build/src/xproof/xproof -k verify

Useful environment variables:
    XPROOF_RUN_E2E=1
    XPROOF_BIN=build/src/xproof/xproof
    XPROOF_TX_HASH=8AF2DE8804721B0EC9E11FD66B9EF3C30962E71DC0D707CA1F4CDE4655751420
    XPROOF_NETWORK=0
    XPROOF_RPC=host:port
    XPROOF_PEER=host:port
"""
...
def config(tmp_path_factory: pytest.TempPathFactory) -> TestConfig:
    if not env_bool("XPROOF_RUN_E2E"):
        pytest.skip(
            "set XPROOF_RUN_E2E=1 or run scripts/xproof-serve-smoke.py directly"
        )
...
def test_concurrent_verify_requests(
    ...
    request_count = env_int("XPROOF_VERIFY_CONCURRENCY") or 8
...
def test_concurrent_prove_requests(
    ...
    request_count = env_int("XPROOF_PROVE_CONCURRENCY") or 2
