...
# ///
"""
Pytest integration suite for `xproof serve` with searchable log artifacts.
...
LOG = logging.getLogger("xproof-serve-smoke-v2")
...
def configure_logging(
    ...
    *,
    ...
    level_text = (
        level_name or os.environ.get("XPROOF_TEST_LOG_LEVEL", "INFO")
    ).upper()
    ...
    if log_path is not None:
        ...
        for handler in root.handlers:
            if isinstance(handler, logging.FileHandler):
                if getattr(handler, "_xproof_log_path", None) == resolved:
        ...
        file_handler._xproof_log_path = resolved  # type: ignore[attr-defined]
...
class RunningServer:
    ...
    def start(self) -> None:
        ...
        if self.proc.stdout is None:
            pytest.fail("failed to capture xproof serve stdout")
        ...
        LOG.info(
            "spawned xproof serve pid=%s bind=%s artifact_dir=%s fd_count=%s",
            self.proc.pid,
            self.config.bind,
    ...
    def stop(self) -> None:
        if self.config.keep_server:
            ...
            print(
                f"[xproof-serve-smoke-v2] leaving server running on {self.config.bind}",
                file=sys.stderr,
            )
            return
    ...
    def _wait_for_port(self) -> None:
        ...
        while time.monotonic() < deadline:
            if self.proc is not None and self.proc.poll() is not None:
                pytest.fail(
                    "xproof serve exited before opening the listening port\n"
        ...
        pytest.fail(
            f"xproof serve never opened {self.config.bind} within "
    ...
    def _wait_for_health(self) -> dict[str, Any]:
        ...
        while time.monotonic() < deadline:
            if self.proc is not None and self.proc.poll() is not None:
                pytest.fail(
                    "xproof serve exited before /health became ready\n"
                    + self._failure_context()
                )
            try:
...
def config(tmp_path_factory: pytest.TempPathFactory) -> TestConfig:
    if not env_bool("XPROOF_RUN_E2E"):
        pytest.skip(
            "set XPROOF_RUN_E2E=1 or run scripts/xproof-serve-smoke-v2.py directly"
        )

    xproof = Path(os.environ.get("XPROOF_BIN", "build/src/xproof/xproof"))
    if not xproof.exists():
        pytest.fail(f"xproof binary not found: {xproof}")

    artifact_env = os.environ.get("XPROOF_ARTIFACT_DIR")
    artifact_dir = (
        Path(artifact_env)
        if artifact_env
        else tmp_path_factory.mktemp("xproof-serve-smoke-v2")
    )
    ...
    bind_address = os.environ.get("XPROOF_BIND_ADDRESS", "127.0.0.1")
    port = env_int("XPROOF_PORT")
    if port is None:
        port = find_free_port(bind_address)

    peer_cache_env = os.environ.get("XPROOF_PEER_CACHE_PATH")
    if peer_cache_env:
    ...
    tx_corpus_env = os.environ.get("XPROOF_TX_CORPUS")
    ...
    cfg = TestConfig(
        ...
        port=port,
        network=env_int("XPROOF_NETWORK") or 0,
        rpc=os.environ.get("XPROOF_RPC"),
        peer=os.environ.get("XPROOF_PEER"),
        ...
        tests_dir=tests_dir,
        startup_timeout=env_float("XPROOF_STARTUP_TIMEOUT", 20.0),
        request_timeout=env_float("XPROOF_REQUEST_TIMEOUT", 120.0),
        shutdown_timeout=env_float("XPROOF_SHUTDOWN_TIMEOUT", 10.0),
        keep_server=env_bool("XPROOF_KEEP_SERVER"),
        mirror_server_output=env_bool("XPROOF_LOG_OUTPUT"),
    ...
    LOG.info(
        "test config: xproof=%s bind=%s network=%s trace_http=%s tx_corpus=%s "
...
def per_test_capture(
    ...
    setattr(request.node, "_xproof_capture", capture)
    LOG.info(
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--xproof",
        help="Path to the xproof binary (maps to XPROOF_BIN)",
    )
    parser.add_argument(
        "--tx-hash",
        help="Transaction hash to prove (maps to XPROOF_TX_HASH)",
    )
    parser.add_argument(
        "--bind-address",
        help="Address to bind the test server to (maps to XPROOF_BIND_ADDRESS)",
    )
    parser.add_argument(
        "--port",
        type=int,
        help="Port to bind the test server to (maps to XPROOF_PORT)",
    )
    parser.add_argument(
        "--network",
        type=int,
        help="Network ID for the server (maps to XPROOF_NETWORK)",
    )
    ...
    parser.add_argument(
        "--shutdown-timeout",
        type=float,
        help="Seconds to wait for xproof serve to exit after SIGINT",
    )
    parser.add_argument(
        "--keep-server",
        action="store_true",
        help="Leave xproof serve running after the test session",
    )
    parser.add_argument(
        "--log-output",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    set_env_if("XPROOF_BIND_ADDRESS", args.bind_address)
    set_env_if("XPROOF_PORT", args.port)
    set_env_if("XPROOF_NETWORK", args.network)
    set_env_if("XPROOF_RPC", args.rpc)
    set_env_if("XPROOF_PEER", args.peer)
    set_env_if("XPROOF_PEER_CACHE_PATH", args.peer_cache_path)
    set_env_if("XPROOF_TX_CORPUS", args.tx_corpus)
    set_env_if("XPROOF_ARTIFACT_DIR", args.artifact_dir)
