...
LedgerHeaderView::LedgerHeaderView(const uint8_t *headerData)
    : data(headerData) {
    // Just store the pointer, no copying
}

uint32_t LedgerHeaderView::sequence() const {
    uint32_t seq;
    std::memcpy(&seq, data + offsetof(LedgerInfo, sequence), sizeof(uint32_t));
    return seq;
}

Hash256 LedgerHeaderView::hash() const {
    return Hash256(data + offsetof(LedgerInfo, hash));
}

Hash256 LedgerHeaderView::parentHash() const {
    return Hash256(data + offsetof(LedgerInfo, parentHash));
}

Hash256 LedgerHeaderView::txHash() const {
    return Hash256(data + offsetof(LedgerInfo, txHash));
}

Hash256 LedgerHeaderView::accountHash() const {
    return Hash256(data + offsetof(LedgerInfo, accountHash));
}

uint32_t LedgerHeaderView::closeTime() const {
    uint32_t time;
    std::memcpy(&time, data + offsetof(LedgerInfo, closeTime), sizeof(uint32_t));
    return time;
}

uint64_t LedgerHeaderView::drops() const {
    uint64_t amount;
    std::memcpy(&amount, data + offsetof(LedgerInfo, drops), sizeof(uint64_t));
    return amount;
}

uint8_t LedgerHeaderView::closeFlags() const {
    uint8_t flags;
    std::memcpy(&flags, data + offsetof(LedgerInfo, closeFlags), sizeof(uint8_t));
    return flags;
}

std::string LedgerHeaderView::toString() const {
...
std::shared_ptr<Ledger> LedgerStore::getLedger(uint32_t sequence) const {
