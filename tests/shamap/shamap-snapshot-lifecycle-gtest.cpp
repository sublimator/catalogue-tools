...
public:
    ...
    static void item_created(const std::string& key_hex) {
        auto& stats = get_stats();
        stats.created++;
        std::lock_guard<std::mutex> lock(stats.mutex);
        stats.live_items[key_hex]++;
        // Don't log individual items - too verbose
        // LOGD("ITEM CREATED: ", key_hex.substr(0, 16), "... (count: ", stats.live_items[key_hex], ")");
    }

    static void item_destroyed(const std::string& key_hex) {
        auto& stats = get_stats();
        stats.destroyed++;
        std::lock_guard<std::mutex> lock(stats.mutex);
        ...
        if (it != stats.live_items.end()) {
            it->second--;
            // Don't log individual items - too verbose
            // LOGD("ITEM DESTROYED: ", key_hex.substr(0, 16), "... (remaining: ", it->second, ")");
            if (it->second == 0) {
                stats.live_items.erase(it);
            }
        } else {
            // This is concerning, so keep this warning
            LOGW("ITEM DESTROYED but not tracked: ", key_hex.substr(0, 16), "...");
        }
    }
    ...
    static void report() {
        ...
        if (items_shown < stats.live_items.size()) {
            LOGI("  ... and ", stats.live_items.size() - items_shown, " more unique items");
        }
        LOGI("Total live item copies: ", total_live);
        LOGI("LEAK STATUS: ", (stats.created.load() == stats.destroyed.load()) ? "NO LEAK" : "MEMORY LEAK!");
        LOGI("=================================");
    }
};
...
public:
    TrackedMmapItem(const uint8_t* key_ptr, const uint8_t* data_ptr, size_t data_size, const std::string& key_hex)
    ...
    ~TrackedMmapItem() {
        LOGI("[TRACKED ITEM DESTRUCTOR] Destroying item with key: ", key_hex_.substr(0, 16), "...");
...
public:
    boost::intrusive_ptr<MmapItem> make_tracked(const std::string& hex_string) {
        ...
        for (size_t i = 0; i < 64; i += 2) {
            ...
            key_bytes.push_back(static_cast<uint8_t>(std::stoi(byte_str, nullptr, 16)));
        }
        ...
        auto item = boost::intrusive_ptr<MmapItem>(
            new TrackedMmapItem(key_ptr, key_ptr, 32, hex_string.substr(0, 64))
        );
    ...
    boost::intrusive_ptr<MmapItem> make_for_ledger(uint32_t ledger, uint32_t item_num) {
        std::ostringstream oss;
...
TEST_F(ShaMapLifecycleFixture, SnapshotMemoryLeakPattern) {
    ...
    for (int ledger = 0; ledger < 10; ++ledger) {
        ...
        // Create snapshot that immediately goes out of scope
        {
            ...
            LOGD("Snapshot for ledger ", ledger, " created, refcount on root should be 2");
            // Snapshot immediately destroyed here
        }
        LOGD("Snapshot for ledger ", ledger, " destroyed, refcount on root should be 1");
    }
...
TEST_F(ShaMapLifecycleFixture, NodeChildrenSharing) {
    ...
    for (int i = 0; i < 20; ++i) {
        auto item = items.make(make_key_hex(i * 1000));  // Sparse to create multiple inner nodes
        parent_map.add_item(item);
    }
    ...
    snapshot1.reset();
    LOGI("First snapshot destroyed - NodeChildren should still be alive if shared");
...
/**
 * Test with instrumented counting (requires modifying destructors to increment counters)
 */
TEST_F(ShaMapLifecycleFixture, CountDestructorCalls) {
    LOGI("Starting CountDestructorCalls test");
    LOGI("NOTE: This test requires modifying destructors to increment counters");
...
TEST_F(ShaMapLifecycleFixture, TrackedItemLifecycle) {
    LOGI("====== Starting TrackedItemLifecycle - TRACKING ITEM LIFETIMES ======");
...

    {
        ...
        for (int i = 0; i < 2; ++i) {
            auto item = tracked_items.make_for_ledger(1, i + 100);  // Different item number to track updates
    ...
    if (stats.created != stats.destroyed) {
        LOGE("MEMORY LEAK DETECTED: Created ", stats.created.load(), " items but only destroyed ", stats.destroyed.load());
    }
}
...
TEST_F(ShaMapLifecycleFixture, TrackedPipelinePattern) {
    LOGI("====== Starting TrackedPipelinePattern - SIMULATING PIPELINE WITH TRACKING ======");
    ...
    for (int ledger = 1; ledger <= 5; ++ledger) {
        ...
        if (ledger > 1) {
            for (int i = 0; i < 2; ++i) {
                auto item = tracked_items.make_for_ledger(ledger - 1, i + 1000);  // Mark as update
                state_map.set_item(item, SetMode::ADD_OR_UPDATE);
            }
        }
...
TEST_F(ShaMapLifecycleFixture, DestructorChainTracking) {
    LOGI("====== Starting DestructorChainTracking - TRACKING DESTRUCTOR CALLS ======");

    // Enable ONLY the destructor partition - no need to change global log level!
    catl::shamap::destructor_log.enable(LogLevel::DEBUG);
    LOGI("Enabled DESTRUCTOR partition logging at DEBUG level (global remains at INFO)");
